...
vec3 integrate_scattered_luminance(vec3 world_position, vec3 world_direction, vec3 sun_direction, int sample_count, float max_integration_length, out vec3 transmittance)
{
    ...
    for (int i = 0; i < sample_count; i++)
    {
        ...
        vec3 shadowPos = new_position - vec3(0, uniforms.sky.atmosphere_bottom + BASE_HEIGHT_OFFSET, 0);
        shadowPos /= M_TO_KM_SCALE;
        shadowPos *= vec3(1, -1, 1);
        shadowPos = shadowPos.xzy; // Y-up conversion
        float cloudShadow = 1;
        //cloudShadow = SampleCascadedBeerShadowMap(shadowPos, uniforms.beerShadowMap);
...
void main()
{
  ...
  const ivec3 outResolution = imageSize(aerialPerspectiveScattering);
  ...
  float z = uv.z;
  //z = pow(z, 3);
  //z = InvertDepthZO(z, 200, 5000000);
  const vec3 rayEndPos               = UnprojectUV_ZO(z, uv.xy, world_from_clip);
  const vec3 rayStartPos             = uniforms.cameraPos.xyz;
  const vec3 rayDir                  = normalize(rayEndPos - rayStartPos);
  const vec3 sunDir                  = uniforms.sky.sunDir;
  const int sampleCount              = 50;
  const float max_integration_length = distance(rayStartPos, rayEndPos) * M_TO_KM_SCALE;

  vec3 transmittance;
  vec3 scattering = integrate_scattered_luminance(rayStartPos * M_TO_KM_SCALE + vec3(0, uniforms.sky.atmosphere_bottom + BASE_HEIGHT_OFFSET, 0),
    rayDir,
    sunDir,
    sampleCount,
  ...
  scattering *= uniforms.sky.sunColor * uniforms.sky.sunBrightness;

  imageStore(aerialPerspectiveTransmittance, gid, vec4(transmittance, 0));
  imageStore(aerialPerspectiveScattering, gid, vec4(scattering, 0));
}