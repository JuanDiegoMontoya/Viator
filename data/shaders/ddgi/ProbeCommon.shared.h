...
// TODO: FIXME: sqrt operation generates NaNs when enabled
#define PERCEPTUAL_BLEND 0
...
vec3 SampleAverageLuminanceRaw(vec3 positionWS, Sampler linearSampler, DDGIArgs ddgi, int cascade, out float outWeight)
{
  // TODO: rename variables in this function to reflect that it's sampling luminance, not illuminance
  if (bool(ddgi.showCascadeIndexAsColor))
  {
    outWeight = 1;
    return TurboColormap(float(cascade) / (DDGI_NUM_CASCADES - 1.0));
  ...
  for (int x = minProbe.x; x <= maxProbe.x; x++)
  {
    ...
    const float normalBias = 0.45 * ddgi.gridInfo[cascade].baseGridScale;
    //const vec3 probeToPointBiasedWS = positionWS - probePosWS + (normal + 3.0 * viewDirWS) * normalBias;
    const vec3 probeToPointBiasedWS = positionWS - probePosWS;
    const vec3 dirToProbeBiased = normalize(-probeToPointBiasedWS);
    const float distToProbeWS = length(probeToPointBiasedWS);
    //const float distToProbeWS = length(probePos - posProbeSpace) * ddgi.gridInfo.baseGridScale;
...
vec3 SampleAverageLuminance(vec3 positionWS, Sampler linearSampler, DDGIArgs ddgi)
{
  // TODO: fix variable names in this function too
  vec3 posProbeSpacePreMod;
  const int cascade = SelectMinimumCascade(positionWS, ddgi, posProbeSpacePreMod);
  if (cascade == -1)
  {
  ...
  const float distFromEdge = clamp(distFromEdgeA < 0 ? -distFromEdgeA - 1 : distFromEdgeA, 0, 1);
  float sumWeight;
  const vec3 lowerCascadeIlluminance = SampleAverageLuminanceRaw(positionWS, linearSampler, ddgi, cascade, sumWeight);
  ...
  float sumWeight2;
  const vec3 upperCascadeIlluminance = SampleAverageLuminanceRaw(positionWS, linearSampler, ddgi, cascade + 1, sumWeight2);
